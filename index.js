...
export default function deepmerge(options) {
	...
	function mergeObject(target, source) {
		...
		for (i = 0, il = sourceKeys.length; i < il; ++i) {
			isNotPrototypeKey((key = sourceKeys[i])) &&
				((key in target &&
					(targetKeys.indexOf(key) !== -1 && (result[key] = _deepmerge(target[key], source[key])), true)) || // eslint-disable-line no-mixed-operators
